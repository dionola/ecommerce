...
const cleanupInvalidCategoryProducts = async () => {
    ...
    `);
        ...
        const productsWithoutImages = allProducts.rows.filter((p) => parseInt(p.image_count) === 0);
        console.log(`\nFound ${productsWithoutImages.length} products without images`);
        if (productsWithoutImages.length > 0) {
            console.log('\nFirst 20 products without images:');
            productsWithoutImages.slice(0, 20).forEach((product) => {
        ...
        const productIdsToDelete = productsWithoutImages.map((p) => p.id);
