...
async function deleteProductsWithoutImages() {
  ...
  const imagesDir = fs.existsSync(path.join(process.cwd(), 'server', 'images'))
    ? path.join(process.cwd(), 'server', 'images')
  ...
  try {
    ...
    for (const product of productsWithImages.rows) {
      ...
      let images: Array<{ id: number; url: string }> = [];
      ...
      if (typeof product.images === 'string') {
        ...
        images = product.images as Array<{ id: number; url: string }>;
      }
    ...
    if (totalToDelete > 0) {
      ...
      const sampleProducts = productsWithImages.rows
        .filter((p: { id: number }) => productsToDelete.includes(p.id))
        .slice(0, 20);
      
      sampleProducts.forEach((product: { id: number; name: string; images: any }) => {
      ...
      if (productsToKeep.length > 0) {
        ...
        productsToKeep.forEach((product: { id: number; name: string; images: any }) => {
          ...
          if (product.images) {
            ...
            if (Array.isArray(imgArray) && imgArray.length > 0) {
              ...
              const imagePath = path.join(imagesDir, `${firstImageId}.jpg`);
              imagesInfo = `${imgArray.length} images, file exists: ${fs.existsSync(imagePath)}`;
            }
          }
          console.log(`   - ID: ${product.id}, Name: ${product.name} (${imagesInfo})`);
        });
