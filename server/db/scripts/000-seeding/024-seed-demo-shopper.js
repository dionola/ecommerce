...
async function ensureDemoShopperData() {
    ...
    try {
        ...
        const userResult = await client.query(`SELECT id FROM users WHERE email = $1`, [DEMO_EMAIL]);
      ...
      `);
        ...
        const wishlistResult = await client.query(`INSERT INTO wishlists (user_id) VALUES ($1) RETURNING id`, [userId]);
        const wishlistId = wishlistResult.rows[0].id;
        const wishlistProducts = products.slice(0, 8);
        for (const product of wishlistProducts) {
            await client.query(`INSERT INTO wishlist_items (wishlist_id, product_id) VALUES ($1, $2)`, [wishlistId, product.id]);
        }
        const cartResult = await client.query(`INSERT INTO carts (user_id) VALUES ($1) RETURNING id`, [userId]);
        const cartId = cartResult.rows[0].id;
        const cartProducts = products.slice(8, 12);
        for (const [index, product] of cartProducts.entries()) {
            await client.query(`INSERT INTO cart_items (cart_id, product_id, quantity) VALUES ($1, $2, $3)`, [cartId, product.id, (index % 2) + 1]);
        }
        const orderGroups = [
